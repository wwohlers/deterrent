...
test('maxLength', () => {
  ...
  expect(() => string().maxLength(4).assert('hello')).toThrowError(
    new ValidationError('Value', 'must be at most 4 characters'),
  );
});
...
test('pattern', () => {
  ...
  expect(() =>
    string()
      .pattern(/^[a-z]+$/)
      .assert('hello123'),
  ).toThrowError(new ValidationError('Value', 'must match the pattern /^[a-z]+$/'));
});
