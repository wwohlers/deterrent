...
test('minLength', () => {
  ...
  expect(() => array().minLength(5).assert([1, 2, 3, 4])).toThrowError(
    new ValidationError('Value', 'must contain at least 5 items'),
  );
});
...
test('multiple calls', () => {
  ...
  expect(() => array().of(string()).minLength(2).assert(['hello'])).toThrowError(
    new ValidationError('Value', 'must contain at least 2 items'),
  );
});
