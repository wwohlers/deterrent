...
test('min', () => {
  ...
  expect(() => number().min(5).assert(4)).toThrowError(new ValidationError('Value cannot be less than 5'));
});

test('max', () => {
  expect(number().max(5).assert(5)).toBe(5);
  expect(number().max(5).assert(4)).toBe(4);
  expect(() => number().max(5).assert(6)).toThrowError(new ValidationError('Value cannot be greater than 5'));
});
...
test('integer', () => {
  ...
  expect(() =>
    number().integer({ errorMessage: 'Value should be a whole number', roundIfNot: false }).assert(4.5),
  ).toThrowError(new ValidationError('Value should be a whole number'));
...
test('multiple calls', () => {
  ...
  expect(() => number().min(5).max(10).assert(4)).toThrowError(new ValidationError('Value cannot be less than 5'));
  expect(() => number().min(5).max(10).assert(11)).toThrowError(new ValidationError('Value cannot be greater than 10'));
...
test('custom', () => {
  expect(
    number()
      .custom((value, error) => {
        if (value < 5) {
          error('Value cannot be less than 5');
        }
        return value;
      })
      .assert(6),
  ...
  expect(() =>
    number()
      ...
      .assert(4),
  ).toThrowError(new ValidationError('Value cannot be less than 5'));
});
